...
public class EnvironmentDepthAccess : MonoBehaviour
{
    ...
    private void UpdateCurrentRenderingState()
    {
        ...
        var leftEyeCamera = _leftEyeCamera;
        var _matrixV = leftEyeCamera.GetStereoViewMatrix(Camera.StereoscopicEye.Left);
        var _matrixVP = GL.GetGPUProjectionMatrix(leftEyeCamera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Left), false) * _matrixV;
        Matrix4x4[] matrixArray = new Matrix4x4[] { _matrixVP.inverse };
        _computeShader.SetMatrixArray("unity_StereoMatrixInvVP", matrixArray);
    }
